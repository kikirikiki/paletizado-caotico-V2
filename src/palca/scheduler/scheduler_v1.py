...
class SchedulerV1:
    ...
    def _hard_floor_phase_rank_group(
        ...
        *,
        ...
        ranked_out = [
            _HardFloorRankedIndex(index=int(item.index), score=float(score), stand_mix_bonus_applied=False)
        ...
        baseline_item, _baseline_score = max(
            ...
            key=lambda pair: (
                float(pair[1]),
                float(pair[0].terms.packing_gain),
                -float(pair[0].terms.fragmentation),
                -float(pair[0].terms.dt_extra),
            ),
        )

...
class SchedulerV1:
        ...
        for ramp_id, ramp in sim_state.ramps.items():
            ...
            for idx, box in enumerate(ramp_items):
                ...
                item_record: dict[str, Any] = {
                    ...
                    "box_length_mm": int(getattr(box, "length_mm", 0) or 0),
                    "box_width_mm": int(getattr(box, "width_mm", 0) or 0),
                    "box_height_mm": int(getattr(box, "height_mm", 0) or 0),
                    "pallet_id": pallet_id,
    ...
    def _scored_candidate_dict(cls, candidate: _ScoredCandidate) -> dict[str, Any]:
        return {
            ...
            "box_length_mm": int(getattr(candidate.box, "length_mm", 0) or 0),
            "box_width_mm": int(getattr(candidate.box, "width_mm", 0) or 0),
            "box_height_mm": int(getattr(candidate.box, "height_mm", 0) or 0),
            "pallet_id": candidate.plan.pallet_id,
    ...
    def _beam_expansion_dict(cls, expansion: _BeamExpansion) -> dict[str, Any]:
        ...
        return {
            ...
            "box_id": plan.box_id,
            "box_length_mm": int(getattr(expansion.box, "length_mm", 0) or 0),
            "box_width_mm": int(getattr(expansion.box, "width_mm", 0) or 0),
            "box_height_mm": int(getattr(expansion.box, "height_mm", 0) or 0),
            "pallet_id": plan.pallet_id,

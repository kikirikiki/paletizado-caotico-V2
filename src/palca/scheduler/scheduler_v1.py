...
class SchedulerConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.z_band_mm is None:
            object.__setattr__(self, "z_band_mm", None)
        else:
            object.__setattr__(self, "z_band_mm", max(0, int(self.z_band_mm)))
...
class SchedulerV1:
        ...
        if micro_enabled:
            ...
            if micro_plan is not None:
                ...
                self.last_eval_stats = {
                    ...
                    "z_band_mm": (
                        int(self.config.z_band_mm) if self.config.z_band_mm is not None else None
                    ),
    ...
    def _choose_action_greedy(
        ...
        *,
        ...
        feasible_candidates, z_band_removed = self._filter_scored_candidates_by_z_band(feasible_candidates)
    ...
    def _choose_action_micro(
        ...
        for depth in range(depth_limit):
            ...
            for node in beam:
                ...
                if depth == 0 and expansions:
                    expansions, removed = self._filter_beam_expansions_by_z_band(expansions)
                    z_band_removed += int(removed)
    ...
    def _preview_z_mm(preview: PlacementPreview | None) -> int | None:
        ...
        if placement is None:
            return None
        try:
            return int(getattr(placement, "z_mm", None))
        except (TypeError, ValueError):
            return None

    def _filter_scored_candidates_by_z_band(
        self,
        candidates: list[_ScoredCandidate],
    ) -> tuple[list[_ScoredCandidate], int]:
        band_raw = self.config.z_band_mm
        if band_raw is None or not candidates:
            return candidates, 0

        min_z_by_pallet: dict[int | str, int] = {}
        for candidate in candidates:
            z_mm = self._preview_z_mm(candidate.plan.preview)
            if z_mm is None:
                continue
            pallet_id = candidate.plan.pallet_id
            current = min_z_by_pallet.get(pallet_id)
            if current is None or z_mm < current:
        ...
        for candidate in candidates:
            ...
            z_mm = self._preview_z_mm(candidate.plan.preview)
            if min_z is None or z_mm is None or int(z_mm) <= int(min_z) + band:
                filtered.append(candidate)
        return filtered, max(0, len(candidates) - len(filtered))

    def _filter_beam_expansions_by_z_band(
        self,
        expansions: list[_BeamExpansion],
    ) -> tuple[list[_BeamExpansion], int]:
        band_raw = self.config.z_band_mm
        if band_raw is None or not expansions:
            return expansions, 0
        ...
        for expansion in expansions:
            ...
            if first_plan is None:
                continue
            z_mm = self._preview_z_mm(first_plan.preview)
            if z_mm is None:
                continue
        ...
        for expansion in expansions:
            ...
            if min_z is None or z_mm is None or int(z_mm) <= int(min_z) + band:
                filtered.append(expansion)
        return filtered, max(0, len(expansions) - len(filtered))

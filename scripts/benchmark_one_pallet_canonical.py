...
DEFAULT_PROFILE_PATH = "configs/benchmarks/one_pallet_canonical.json"
...
REQUIRED_PARAM_KEYS = RUN_SIMULATION_PARAM_KEYS - RUN_SIM_EXCLUDED_PROFILE_KEYS


def _validate_harness_contract() -> None:
    required = {"lookahead_k", "overhang_mm", "time_budget_ms", "height_slack_mm"}
    missing = sorted(required - RUN_SIMULATION_PARAM_KEYS)
    if missing:
        raise RuntimeError(
            f"Harness contract changed: run_simulation no longer exposes expected keys: {missing}"
        )
...
def _safe_get(data: dict[str, Any], path: list[str], default: Any = None) -> Any:
    ...
    for key in path:
        ...
        try:
            ...
            key_int = None
        if key_int is not None and key_int in current:
            current = current.get(key_int)
            continue
        return default
    return current
...
def _parse_set_overrides(raw_items: list[str] | None) -> dict[str, Any]:
    out: dict[str, Any] = {}
    for item in raw_items or []:
        ...
        if not token:
            continue
        if "=" not in token:
            raise ValueError(f"Override invalido (esperado key=value): {token!r}")
...
def load_profile(path: str | Path) -> dict[str, Any]:
    ...
    missing_required_args = sorted(
        ...
        if param.default is inspect._empty and name not in RUN_SIM_EXCLUDED_PROFILE_KEYS and name not in params
    )
...
def apply_param_overrides(base_params: dict[str, Any], overrides: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in overrides.items():
        if key not in REQUIRED_PARAM_KEYS:
            raise ValueError(f"Override invalido: parametro desconocido/no usado por run_simulation '{key}'")
        out[key] = value
    return out
...
def _extract_placement_sequence(dump_path: Path, *, forced_destination: int | None = None) -> list[dict[str, Any]]:
    ...
    for key in pallets.keys():
        try:
            ...
            sortable_keys.append((10**9, str(key)))
...
def _write_step_trace_report(path: Path, rows: list[dict[str, Any]]) -> None:
    header = [
        "step",
        "z_mm",
        "max_z_so_far",
        ...
        "reentry_drop_mm",
        "active_layers",
        "below_top_band_after_opening_next",
    ]
    lines = [" | ".join(header)]
    lines.append("-" * 112)
    for row in rows:
        lines.append(
            " | ".join(
                [
                    str(_safe_int(row.get("step")) or 0),
                    str(_safe_int(row.get("z_mm")) or 0),
                    str(_safe_int(row.get("max_z_seen_so_far")) or 0),
                    str(bool(row.get("opened_new_band", False))),
                    str(bool(row.get("is_reentry", False))),
                    str(_safe_int(row.get("reentry_drop_mm")) or 0),
                    str(_safe_int(row.get("active_layers")) or 0),
                    str(bool(row.get("below_current_top_band_after_opening_next_band", False))),
                ]
            )
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run_seed(
    *,
    ...
    trace_report_path = run_dir / f"seed_{int(seed)}_layer_trace.txt"
    ...
    hard_floor_stand_total = (
        _safe_int(pallet_kpis.get("hard_floor_phase_stand_hw_chosen_total")) if isinstance(pallet_kpis, dict) else None
    )
    ...
    mono = _select_first_pallet_monotonicity(pallet_kpis if isinstance(pallet_kpis, dict) else {}, forced_destination=forced_destination)

    first_stack_from_kpi = _safe_int(mono.get("first_stack_step"))
    if first_stack_from_kpi is not None:
        first_stack_step = first_stack_from_kpi
    ...
    if not isinstance(step_trace_relevant, list):
        step_trace_relevant = []
    clean_trace = [item for item in step_trace_relevant if isinstance(item, dict)]
    _write_step_trace_report(trace_report_path, clean_trace)
    ...
    return SeedSummary(
        ...
        active_layers_over_time_json=json.dumps(active_layers_series if isinstance(active_layers_series, list) else [], ensure_ascii=True),
        ...
        layer_trace_report=str(trace_report_path),
...
def _aggregate_rows(rows: list[SeedSummary]) -> dict[str, dict[str, Any]]:
    ...
    for label, values in by_label.items():
        ...
        out[label] = {
            ...
            "processed_boxes_min": (
                min(v.processed_boxes for v in values_sorted if v.processed_boxes is not None)
                if any(v.processed_boxes is not None for v in values_sorted)
                else None
            ),
            "processed_boxes_max": (
                max(v.processed_boxes for v in values_sorted if v.processed_boxes is not None)
                if any(v.processed_boxes is not None for v in values_sorted)
                else None
            ),
        }
...
def _default_outdir(*, profile: dict[str, Any], variant_name: str, seeds: list[int], baseline_hash: str) -> Path:
    safe_profile = "".join(ch if (ch.isalnum() or ch in "-_") else "_" for ch in str(profile["profile_name"]))
    token = _stable_hash(
        {
    ...
    return Path("out") / "benchmarks" / f"{safe_profile}_{token}"
        ...
        print(
            " | ".join(
                [
                    ...
                    str(row.lower_layer_reentry_count),
                    str(row.lower_layer_reentry_max_drop_mm),
                    str(row.monotonic_stack_rate),
                    str(row.layer_closure_score),
                ]
            )
        )
...
def run_benchmark(
    *,
    ...
    baseline_missing_params = sorted(
        name
        for name, param in RUN_SIMULATION_SIGNATURE.parameters.items()
        if param.default is inspect._empty
        and name not in RUN_SIM_EXCLUDED_PROFILE_KEYS
        and name not in baseline_params
    )
    ...
    baseline_hash = _stable_hash({"excel": baseline_excel, "params": baseline_params})
    variant_requested = bool(variant_excel_override) or bool(merged_variant_overrides)
    ...
    variant_hash = _stable_hash({"excel": variant_excel, "params": variant_params})

    run_output_dir = (
        Path(outdir).expanduser().resolve()
        if outdir
        else _default_outdir(
            profile=profile,
            variant_name=str(variant_name),
            seeds=seeds,
    ...
    runs: list[tuple[str, str, dict[str, Any], str]] = [("baseline", baseline_excel, baseline_params, str(baseline_hash))]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            "Path to canonical benchmark profile "

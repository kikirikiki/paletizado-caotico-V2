...
class SeedSummary:
    ...
    closure_reservation_hits: int | None
    closure_reservation_misses: int | None
...
def run_seed(
    *,
    ...
    blocked_upper_layer_open_attempts = (
        _safe_int(pallet_kpis.get("blocked_upper_layer_open_attempts")) if isinstance(pallet_kpis, dict) else None
    )
    closure_reservation_hits = (
        _safe_int(pallet_kpis.get("closure_reservation_hits")) if isinstance(pallet_kpis, dict) else None
    )
    closure_reservation_misses = (
        _safe_int(pallet_kpis.get("closure_reservation_misses")) if isinstance(pallet_kpis, dict) else None
    )
    ...
    return SeedSummary(
        ...
        closure_reservation_hits=closure_reservation_hits,
        closure_reservation_misses=closure_reservation_misses,
...
def _aggregate_rows(rows: list[SeedSummary]) -> dict[str, dict[str, Any]]:
    ...
    for label, values in by_label.items():
        ...
        out[label] = {
            ...
            "closure_reservation_hits_mean": _mean([v.closure_reservation_hits for v in values_sorted]),
            "closure_reservation_misses_mean": _mean([v.closure_reservation_misses for v in values_sorted]),
